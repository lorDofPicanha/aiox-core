 ...
 */
function alternarPrimeiraPessoa(raiz) {
    for (let i = raiz.length - 1; i >= 0; i--) {
        const c = raiz[i];
        if (c === 'e')
            return raiz.slice(0, i) + 'i' + raiz.slice(i + 1);
        if (c === 'o')
            return raiz.slice(0, i) + 'u' + raiz.slice(i + 1);
        if ('aiu'.includes(c))
            return raiz;
    }
    return raiz;
...
function conjugar(infinitivo) {
    const inf = normalizar(infinitivo);
    const irregular = IRREGULARES[inf];
    if (irregular)
        return irregular;
    const raiz = inf.slice(0, -2);
    const formas = new Set([inf]);
    const add = (sufixos) => sufixos.forEach((s) => formas.add(raiz + s));
    if (inf.endsWith('ar')) {
...
function pluralizar(base) {
    const formas = [base, `${base}s`];
    if (base.endsWith('ao'))
        formas.push(`${base.slice(0, -2)}oes`);
    return formas;
}
const v = (base, q) => ({
    especie: 'verbo',
...
const p = (base) => ({ especie: 'palavra', base });
const f = (base) => ({ especie: 'frase', base });
...
const FORMAS_VERBAIS = new Set();
REGRAS.forEach((regra, ordem) => {
    for (const termo of regra.termos) {
        const entrada = { regra, termo, ordem };
        ...
        for (const forma of formas) {
            if (termo.especie === 'verbo')
                FORMAS_VERBAIS.add(forma);
            const lista = INDICE.get(forma);
            if (lista)
                lista.push(entrada);
            else
                INDICE.set(forma, [entrada]);
        }
    }
});
function temVerbo(trecho, maxTokens) {
    const tokens = tokenizar(trecho);
    const ate = Math.min(tokens.length, maxTokens);
    for (let i = 0; i < ate; i++) {
        if (FORMAS_VERBAIS.has(tokens[i].texto))
            return true;
    }
    return false;
}
function cortar(frag, pontos) {
    if (pontos.length === 0)
        return [frag];
    const pedacos = [];
    let inicio = 0;
    for (const ponto of pontos) {
        if (ponto > inicio)
            pedacos.push(frag.slice(inicio, ponto));
        inicio = ponto;
    }
...
function cortarPorConector(frag) {
    const plano = normalizar(frag);
    if (plano.length !== frag.length)
        return [frag];
...
function cortarPorVerbo(frag) {
    ...
    while ((m = re.exec(plano)) !== null) {
        ...
        if (!temVerbo(plano.slice(inicio, fimEsquerda), Infinity))
            continue;
        if (!temVerbo(plano.slice(inicioDireita), 3))
            continue;
 ...
 */
const PESO = { qualificado: 3, frase: 1, simples: 1 };
/**
...
function classificar(bruto) {
    ...
    for (const { regra, re, ordem } of FRASES) {
        ...
        if (pos === -1)
            continue;
        const achado = { regra, peso: PESO.frase, pos, ordem };
        melhor = melhor ? melhorAchado(melhor, achado) : achado;
    }
    if (!melhor) {
...
/* ────────────────────────────────────────────────────────────────────────────
   O motor.
   ──────────────────────────────────────────────────────────────────────────── */
export function mapear(texto, vezesPorSemana) {
    const trace = [];
    const avisos = [];
    ...
    if (brutas.length > 0 && densidade > PALAVRAS_POR_ETAPA_SUSPEITO) {
        detalheSeparacao.push(`${Math.round(densidade)} palavras por etapa — provavelmente li junto o que acontece separado`);
        avisos.push({
            tipo: 'subsegmentacao',
            texto: brutas.length === 1
    ...
    t = performance.now();
    const etapas = brutas.map((b, i) => ({ n: i + 1, texto: b, ...classificar(b) }));
    ...
    const minutosExecucao = etapas.reduce((soma, e) => {
        if (e.vereditoReal === 'automatizavel')
            return soma + e.minutos;
        // Parcial devolve metade: a máquina redige, alguém ainda confere.
        if (e.vereditoReal === 'parcial')
            return soma + e.minutos / 2;
    ...
    if (naoLidas > 0)
        detalheEstimativa.push(plural(naoLidas, 'etapa fora da conta', 'etapas fora da conta'));
...
export const ROTULO_VEREDITO_REAL = {
    ...ROTULO_VEREDITO,
    nao_lida: 'não li',
};
export const EXEMPLO = 'O cliente manda mensagem no WhatsApp pedindo orçamento. Alguém lê e responde perguntando o que ele precisa. Depois copia os dados pra planilha de leads. Aí confere o preço na tabela e monta a proposta no Word. Por fim avisa o vendedor que tem proposta nova.';
