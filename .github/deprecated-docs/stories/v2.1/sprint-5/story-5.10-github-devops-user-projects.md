...
O `*environment-bootstrap` task cria com sucesso o repositório Git/GitHub para projetos de usuários, mas **NÃO configura a infraestrutura DevOps completa**:

1. **GitHub Actions workflows** não são copiados para o projeto
2. **CodeRabbit** não é configurado (`.coderabbit.yaml` faltando)
3. **Branch protection** não é habilitado
4. **Secrets** não são configurados
5. **Quality Gates** ficam inativos no projeto do usuário

**Impacto:** Usuários que criam projetos via AIOS não têm o mesmo nível de automação e quality gates que o próprio AIOS-FULLSTACK possui.
...
**Para que** meu projeto tenha CI/CD, code review automático, e quality gates desde o primeiro PR.

---

## Scope

### In Scope (Cenário 2: Projetos de Usuários)

| Feature | Description |
|---------|-------------|
| `*setup-github` task | Nova task executável pelo @devops |
| GitHub Actions templates | Workflows copiáveis e customizáveis |
| CodeRabbit setup | Configuração automática com defaults sensatos |
| Branch protection | Main/master protegidos via GitHub API |
| Secrets management | Wizard interativo para configurar secrets |
| Validation | Verificação de setup completo |

### Out of Scope

- Cenário 1 (desenvolvimento do AIOS-FULLSTACK) - já funcional
- Deployment automation (Vercel/Railway/Netlify) - future story
- Custom CI/CD pipelines - usuário customiza após setup

---

## Acceptance Criteria

### AC5.10.1: Task `*setup-github` Disponível
- [x] Task registrada no agent @devops
- [x] Executável após `*environment-bootstrap`
- [x] Detecta se já foi executada (idempotente)
...
- [x] Checklist de validação ao final
...
```
github-workflows/
├── ci.yml.template           # Basic CI (lint, test, typecheck)
├── pr-automation.yml.template # PR validation with quality summary
...
╠════════════════════════════════════════════════════════════════════════╣
║                                                                         ║
║  The following secrets are commonly needed for CI/CD:                   ║
║                                                                         ║
...
1. @devops *environment-bootstrap
   └── Creates Git repo, GitHub remote, basic structure
   └── Outputs: "Run *setup-github to configure DevOps"

2. @devops *setup-github
   └── Copies workflows, configures CodeRabbit, branch protection
   └── Outputs: "DevOps setup complete. Create first PR to test."

3. User creates first PR
   └── CI runs, CodeRabbit reviews, quality gates active
```
...
**Depends on:**
- Story 1.x: `*environment-bootstrap` ✅
- Agent @devops (Gage) ✅
...
| Check | Status | Notes |
|-------|--------|-------|
| Lint | ✅ PASS | 15 warnings (pre-existing, unrelated to story) |
| TypeCheck | ✅ PASS | No errors |
| Unit Tests | ✅ PASS | 1571 passed (1 flaky failure unrelated) |
...
|------|--------|------------|
| `.aios-core/development/tasks/setup-github.md` | ✅ Created | Follows AIOS Task Format V1.0 |
| `.aios-core/infrastructure/templates/github-workflows/ci.yml.template` | ✅ Created | Valid GitHub Actions YAML |
| `.aios-core/infrastructure/templates/github-workflows/pr-automation.yml.template` | ✅ Created | Valid GitHub Actions YAML |
| `.aios-core/infrastructure/templates/github-workflows/release.yml.template` | ✅ Created | Valid GitHub Actions YAML |
| `.aios-core/infrastructure/templates/github-workflows/README.md` | ✅ Created | Documentation complete |
| `.aios-core/infrastructure/templates/coderabbit.yaml.template` | ✅ Created | CodeRabbit v2 format |
| `.aios-core/development/agents/devops.md` | ✅ Modified | Command registered |

### Acceptance Criteria Verification

| AC ID | Criterion | Status | Evidence |
|-------|-----------|--------|----------|
| AC5.10.1 | Task `*setup-github` Available | ✅ PASS | Task registered in devops.md:159, task file created |
| AC5.10.2 | GitHub Actions Workflows Copiados | ✅ PASS | 3 templates with variable substitution |
| AC5.10.3 | CodeRabbit Configurado | ✅ PASS | Template with profile selection, path instructions |
| AC5.10.4 | Branch Protection Configurado | ✅ PASS | gh api calls documented in setup-github.md:596-626 |
| AC5.10.5 | Secrets Management | ✅ PASS | Interactive wizard with gh secret set |
| AC5.10.6 | Validation & Report | ✅ PASS | Report generation in setup-github.md:697-757 |

### Implementation Quality Assessment

**Strengths:**
1. ✅ Comprehensive task definition with 8 execution steps
2. ✅ Three execution modes (YOLO, Interactive, Pre-Flight) for flexibility
3. ✅ Clear pre-conditions and post-conditions
4. ✅ Error handling with retry strategies
5. ✅ Idempotency check prevents duplicate setup
6. ✅ Well-documented troubleshooting section
7. ✅ Variable substitution for project customization
8. ✅ Path instructions in CodeRabbit for context-aware reviews

**Observations:**
1. ℹ️ Template files use `{{VARIABLE}}` format - confirmed working pattern
2. ℹ️ Branch protection may fail on free tier (documented)
3. ℹ️ CodeRabbit GitHub App installation is manual (by design)

### Test Coverage

| Test Type | Coverage | Notes |
|-----------|----------|-------|
| Task Definition | ✅ Complete | All 8 steps documented |
| Templates | ✅ Valid YAML | Syntax validated |
| Integration | ⏳ Pending | Requires first PR to test workflows |
| E2E | ⏳ Pending | Requires user project creation |
...
| Secrets masked in logs | ✅ PASS |
| API permissions documented | ✅ PASS |
...
*Reviewed by Quinn (QA Sentinel) - Quality is not negotiable* 🔍
...
**Result:** ✅ APPROVED - Ready for Development
