-- Rollback for 003_secure_decision_rpc.sql.
-- Restores the F1.1 human decision RPC contract from 002_decision_evidence.sql.
...
drop function if exists core_api_v1.assert_human_decision_context(uuid, uuid, text);
drop function if exists core_api_v1.decision_evidence_payload(uuid, uuid, text, text, jsonb);
