...
// Auto-load the repo .env (zero-dep) so OPENROUTER_API_KEY etc. are picked up without
// manual export — keeps design-md off the metered `claude -p` fallback when a key exists.
(function loadEnv() {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (_fs.existsSync(p)) {
        for (const ln of _fs.readFileSync(p, "utf8").split(/\r?\n/)) {
          const s = ln.trim();
