...
interface ClienteMeta {
  ...
  documento: string;
  modo: ModoCaptura;
  /** Toggle visual; default OFF é a decisão D2 (captura seletiva). */
 ...
 */
const NOTAS: Omit<NotaCapturadaLinha, "clienteNome">[] = [
...
/** Lista de notas capturadas, ordenada por data desc (mais recente no topo). */
export function listarNotasCapturadas(): NotaCapturadaLinha[] {
  return NOTAS.map((n) => ({ ...n, clienteNome: nomeById.get(n.clienteId) ?? "—" })).sort(
...
function classePredominante(clienteId: string): ClasseInsumo {
  let xml = 0;
  let ocr = 0;
...
/** Data da última captura de um cliente (ISO) — null se não há nota nem captura ativa. */
function ultimaCaptura(clienteId: string, capturaAtiva: boolean): string | null {
  const datas = NOTAS.filter((n) => n.clienteId === clienteId).map((n) =>
    new Date(n.dataIso).getTime(),
  );
  if (datas.length === 0) return null;
  // Captura OFF mas com notas históricas (ex.: upload manual anterior): mostra a última.
  void capturaAtiva;
  return new Date(Math.max(...datas)).toISOString();
}

/**
 * Status de captura POR CLIENTE — derivado das notas (uma fonte só). Inclui o toggle
 * visual capturaAtiva (default OFF = D2) e a classe de insumo predominante.
 */
export function statusCapturaPorCliente(): CapturaClienteLinha[] {
  return CLIENTES.map((c): CapturaClienteLinha => ({
    ...
    ultimaCapturaIso: ultimaCaptura(c.id, c.capturaAtiva),
    classePredominante: classePredominante(c.id),
  }));
}
