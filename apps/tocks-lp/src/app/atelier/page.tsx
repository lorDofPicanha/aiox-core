  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 text-center">
        ...
        <span className="text-accent text-sm uppercase tracking-[0.3em] mb-6 relative">
          Desde {COMPANY.foundingYear}
        </span>
        <h1 className="font-heading text-6xl md:text-7xl lg:text-[5.5rem] font-semibold text-text-primary leading-[0.95] relative">
          Nosso Atelier
        </h1>
        <p className="mt-6 text-lg md:text-xl text-text-secondary max-w-md relative">
          Desde 1988, Itajaí — Santa Catarina
        </p>

        {/* Scroll indicator */}
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-px h-16 overflow-hidden">
          <div className="w-full h-full bg-accent animate-[scroll-pulse_2s_ease-in-out_infinite]" />
        </div>
      </section>
      ...
      <section className="py-32 md:py-48 px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 md:gap-24 items-center">
          ...
          <ScrollReveal>
            <div>
              <span className="text-accent text-sm uppercase tracking-[0.2em]">
                A Origem
              </span>
              <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-semibold text-text-primary mt-4 leading-tight">
                Uma história escrita em madeira
              </h2>
              <div className="mt-8 space-y-6 text-text-secondary text-base md:text-lg leading-relaxed">
                <p>
      ...
      <section className="py-16 border-y border-border">
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          <div>
            <Counter end={38} suffix=" anos" className="font-heading text-4xl md:text-5xl font-semibold text-accent" />
            <p className="text-text-secondary text-sm mt-2">de tradição</p>
          </div>
          <div>
            <Counter end={250} prefix="+" className="font-heading text-4xl md:text-5xl font-semibold text-accent" />
            <p className="text-text-secondary text-sm mt-2">mesas entregues</p>
          </div>
          <div>
            <Counter end={14} className="font-heading text-4xl md:text-5xl font-semibold text-accent" />
            <p className="text-text-secondary text-sm mt-2">modelos exclusivos</p>
          </div>
          <div>
            <Counter end={12} className="font-heading text-4xl md:text-5xl font-semibold text-accent" />
            <p className="text-text-secondary text-sm mt-2">camadas de verniz</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 md:py-48 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-20">
            ...
            <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-semibold text-text-primary mt-4">
          ...
          <ScrollReveal className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12" stagger={0.2}>
            {MILESTONES.map((m) => (
              <div key={m.year} className="text-center md:text-left">
                <span className="font-heading text-4xl md:text-5xl font-semibold text-accent block">
                  {m.year}
                </span>
                <div className="w-8 h-px bg-accent/40 my-4 mx-auto md:mx-0" />
      ...
      {/* Values */}
      <section className="py-32 md:py-48 px-6 bg-bg-secondary">
        <div className="max-w-5xl mx-auto">
          ...
          <ScrollReveal className="grid md:grid-cols-3 gap-8 md:gap-12" stagger={0.15}>
            {VALUES.map((v) => (
              <div
                key={v.title}
                className="bg-bg-card border border-border rounded-lg p-8 md:p-10 text-center"
              >
                <div className="w-14 h-14 rounded-full border border-accent/30 flex items-center justify-center mx-auto mb-6 text-accent">
