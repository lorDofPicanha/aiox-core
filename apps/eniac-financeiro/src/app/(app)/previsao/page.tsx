...
export default async function PrevisaoPage() {
  ...
  const now = new Date();
  const [summary, scheduled] = await Promise.all([
    getMonthSummary(selected.id, now.getFullYear(), now.getMonth() + 1),
  ...
  return (
    <main className="px-4 pb-28 pt-4">
      <header className="mb-4 flex items-center gap-2">
        ...
        <h1 className="text-lg font-bold">Previsão de caixa</h1>
      </header>
