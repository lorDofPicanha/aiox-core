...
import { listBankConnections } from "@/lib/openfinance/connections";
...
export default async function ConexoesPage() {
  ...
  const connections = await listBankConnections(selected.id);
  ...
  return (
    <main className="pb-nav px-4 pt-4">
      <header className="mb-4 flex items-center gap-2">
        ...
        <h1 className="text-lg font-bold">Conexões bancárias</h1>
      </header>
      ...
      <ConnectionsClient companyId={selected.id} connections={connections} />
    </main>
  );
}
